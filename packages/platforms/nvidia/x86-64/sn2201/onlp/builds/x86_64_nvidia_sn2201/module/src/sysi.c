...
{
    gPlat_id = PID_SN2201;
    return "x86-64-nvidia-sn2201";
}

int
onlp_sysi_platform_set(const char* platform)
{
    if (strstr(platform, "x86-64-nvidia-sn2201-r0")) {
        gPlat_id = PID_SN2201;
...
onlp_sysi_platform_info_get(onlp_platform_info_t* pi)
{
    ...
    /* CPLD Version */
    sprintf(fullpath, "%s/cpld%d_version", plat_info->cpld_path, 1);
...
onlp_sysi_platform_manage_fans(void)
{
    ...
    for (i = PLAT_FAN_ID_1; i <= PLAT_FAN_ID_4 ; i ++) {
...
onlp_sysi_platform_manage_leds(void)
{
    ...
    if (fan_new_mode == ONLP_LED_MODE_GREEN &&
        ...
        sys_new_mode = ONLP_LED_MODE_YELLOW;
