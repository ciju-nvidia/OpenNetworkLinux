...
plat_info_t gPlat_info[] = {
    [PID_SN2201] = {
        .name = "x86-64-nvidia-sn2201",

        .onie_eeprom_path = "/var/run/hw_managemant/eeprom/vpd_info",

        .cpld_bus = 1,
        .cpld_path = "/var/run/hw_managemant/system",
...
plat_id_t get_platform_id(void)
{
    ...
    plat_info_t *sn2201_info = &gPlat_info[PID_SN2201];

    sprintf(pid_fullpath, "%s/platform_id", sn2201_info->cpld_path);
