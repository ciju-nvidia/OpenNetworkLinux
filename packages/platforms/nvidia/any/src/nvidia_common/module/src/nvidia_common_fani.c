...
_onlp_fani_info_get_fan(int local_id, onlp_fan_info_t* info)
{
    ...
    nvidia_platform_info_t* nvidia_platform_info = get_platform_info();
    if(nvidia_platform_info->fan_type == FAN_TYPE_NO_EEPROM)
        aim_strlcpy(info->model, fan_model, sizeof(info->model));

    if(!nvidia_platform_info->fan_fixed) {
      /* not fixed FAN's can have more than 1 FAN per FRU.
         EEPROM is per FRU but not per FAN.
         So, need to convert fan ID to FRU ID.*/
        if (nvidia_platform_info->fan_per_module == 0) {
            info->status |= ONLP_FAN_STATUS_FAILED;
            return ONLP_STATUS_OK;
        }

        /* get fan status */
        fru_index = (local_id + nvidia_platform_info->fan_per_module -1) / nvidia_platform_info->fan_per_module;

        ret = onlp_file_read_int(&r_val, "%s/%s", THERMAL_PATH, nvidia_platform_info->fan_fnames[fru_index].status);
        if (ret < 0) {
            return ONLP_STATUS_E_INTERNAL;
        }
        if (r_val != FAN_STATUS_OK) {
            if(nvidia_platform_info->fan_type == FAN_TYPE_EEPROM)
    ...
    /* get fan speed */
    ret = onlp_file_read_int(&r_val, "%s/%s", THERMAL_PATH, nvidia_platform_info->fan_fnames[local_id].r_speed_get);
    if (ret < 0) {
        return ONLP_STATUS_E_INTERNAL;
    }
    info->rpm = r_val;
    ...
    if (ONLP_FAN_CAPS_GET_PERCENTAGE & info->caps) {
        /* get fan min speed */
        ret = onlp_file_read_int(&r_val, "%s/%s", THERMAL_PATH, nvidia_platform_info->fan_fnames[local_id].min);
        if (ret < 0) {
            return ONLP_STATUS_E_INTERNAL;
        }
        nvidia_platform_info->min_fan_speed[local_id] = r_val;

        /* get fan max speed */
        ret = onlp_file_read_int(&r_val, "%s/%s", THERMAL_PATH, nvidia_platform_info->fan_fnames[local_id].max);
        if (ret < 0) {
            return ONLP_STATUS_E_INTERNAL;
        }
        nvidia_platform_info->max_fan_speed[local_id] = r_val;

        /* get speed percentage from rpm */
        range = nvidia_platform_info->max_fan_speed[local_id] - nvidia_platform_info->min_fan_speed[local_id];
        if (range > 0) {
            temp = ((float)info->rpm - (float)nvidia_platform_info->min_fan_speed[local_id]) / range * 40.0 + 60.0;
...
_onlp_fani_info_get_fan_on_psu(int local_id, int psu_id, onlp_fan_info_t* info)
{
    ...
    /* get fan status
    */
    ret = onlp_file_read_int(&r_val, "%s/%s", THERMAL_PATH, nvidia_platform_info->fan_fnames[local_id].status);
    if (ret < 0) {
        return ONLP_STATUS_E_INTERNAL;
    }
    ...
    */
    ...
    if (0 != psu_fan_read_eeprom_vpd((local_id-nvidia_platform_info->first_psu_fan_id)+1, info))
        return ONLP_STATUS_E_INTERNAL;
...
onlp_fani_info_get(onlp_oid_t id, onlp_fan_info_t* info)
{
    ...
    *info = nvidia_platform_info->finfo[local_id];

    if(local_id<nvidia_platform_info->first_psu_fan_id)
      rc =_onlp_fani_info_get_fan(local_id, info);
    else
      rc = _onlp_fani_info_get_fan_on_psu(local_id, (local_id-nvidia_platform_info->first_psu_fan_id)+1, info);
    return rc;
}
...
onlp_fani_rpm_set(onlp_oid_t id, int rpm)
{
    ...
    */
    if (local_id > nvidia_platform_info->fan_num) {
        return ONLP_STATUS_E_INTERNAL;
    }
    if (nvidia_platform_info->max_fan_speed[local_id] - nvidia_platform_info->min_fan_speed[local_id] < 0) {
        return ONLP_STATUS_E_INTERNAL;
    }
    if (rpm < nvidia_platform_info->min_fan_speed[local_id] || rpm > nvidia_platform_info->max_fan_speed[local_id]) {
        return ONLP_STATUS_E_PARAM;
    }

    temp = (rpm - nvidia_platform_info->min_fan_speed[local_id]) * (RPM_MAGIC_MAX - RPM_MAGIC_MIN) /
           (nvidia_platform_info->max_fan_speed[local_id] - nvidia_platform_info->min_fan_speed[local_id]) + RPM_MAGIC_MIN;
    ...
    rv = onlp_file_write((uint8_t*)r_data, nbytes, "%s/%s", THERMAL_PATH,
                         nvidia_platform_info->fan_fnames[local_id].r_speed_set);
    if (rv < 0) {
        return ONLP_STATUS_E_INTERNAL;
    }
...
onlp_fani_get_min_rpm(int id)
{
    ...
    if (onlp_file_read_int(&r_val, "%s/%s", THERMAL_PATH, nvidia_platform_info->fan_fnames[id].min) < 0)
        return ONLP_STATUS_E_INTERNAL;
