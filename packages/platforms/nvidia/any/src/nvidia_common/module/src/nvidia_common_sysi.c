...
#include "nvidia_common_int.h"
#include "nvidia_common/nvidia_common.h"

nvidia_platform_info_t nvidia_platform_info;
...
nvidia_platform_info_t* get_platform_info()
{
...
{
	if (mc_get_platform_info(&nvidia_platform_info) < 0) {
		AIM_LOG_ERROR("Unable to get paltform info!\n");
		return NULL;
	}
	else
		return nvidia_platform_info.onl_platform_name;
}
...
onlp_sysi_platform_info_get(onlp_platform_info_t* pi)
{
    ...
    nvidia_platform_info_t* platform_info = get_platform_info();
...
onlp_sysi_oids_get(onlp_oid_t* table, int max)
{
    ...
    for (i = 1; i <= nvidia_platform_info.thermal_num; i++)
    {
        *e++ = ONLP_THERMAL_ID_CREATE(i);
    }

    for (i = 1; i <= nvidia_platform_info.led_num; i++)
    {
        *e++ = ONLP_LED_ID_CREATE(i);
    }

    for (i = 1; i <= nvidia_platform_info.psu_num; i++)
    {
        *e++ = ONLP_PSU_ID_CREATE(i);
    }

    for (i = 1; i <= nvidia_platform_info.fan_num; i++)
    {
...
onlp_sysi_onie_info_get(onlp_onie_info_t* onie)
{
    ...
    if(rv >= 0) {
        ...
        onie->platform_name = aim_strdup(nvidia_platform_info.onl_platform_name);
    }
...
onlp_sysi_platform_manage_leds_type1(void)
{
	...
	for( fan_number = 1; fan_number<= nvidia_platform_info.fan_num; fan_number+=2)
	{
		...
		if(onlp_fani_info_get(ONLP_FAN_ID_CREATE(fan_number), &fi) < 0) {
		    ...
		    if(nvidia_platform_info.fan_fixed == false) {
			/* Not present */
			mode = ONLP_LED_MODE_RED;
			fan_problem = 1;
		    }
	...
	for (psu_number = 1; psu_number <= nvidia_platform_info.psu_num; psu_number++)
	{
		onlp_psu_info_t pi;
		mode = ONLP_LED_MODE_GREEN;
		if(onlp_psui_info_get(ONLP_PSU_ID_CREATE(psu_number), &pi) < 0) {
		    ...
		    if(nvidia_platform_info.psu_fixed) {
...
onlp_sysi_platform_manage_leds_type3(void)
{
     ...
     */
    fan_idx=0;
    for (fan_number = 1; fan_number <= nvidia_platform_info.fan_num; fan_idx++)
    {
        onlp_fan_info_t fi;
        mode = ONLP_LED_MODE_GREEN;
        for (fan_sub = 0; fan_sub < nvidia_platform_info.fan_per_module; fan_sub++, fan_number++) {
            /* check fans */
	        if (onlp_fani_info_get(ONLP_FAN_ID_CREATE(fan_number), &fi) < 0)
	        {
...
{
    int res;
    switch (nvidia_platform_info.led_type) {
        case LED_TYPE_1:
