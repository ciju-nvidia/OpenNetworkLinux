...
static int led_set_mode(onlp_oid_t id, onlp_led_mode_t mode)
{
    ...
    if (blinking) {
        onlp_file_write((uint8_t*)LED_BLINK_PERIOD, LED_BLINK_PERIOD_LEN,
                        "%s%s_%s_delay_off", prefix_path, nvidia_platform_info->led_fnames[local_id], color);
        onlp_file_write((uint8_t*)LED_BLINK_PERIOD, LED_BLINK_PERIOD_LEN,
                        "%s%s_%s_delay_on", prefix_path, nvidia_platform_info->led_fnames[local_id], color);
    }
    onlp_file_write((uint8_t*)LED_ON, LED_MODE_LEN,
                    "%s%s_%s", prefix_path, nvidia_platform_info->led_fnames[local_id], color);
...
onlp_ledi_info_get(onlp_oid_t id, onlp_led_info_t* info)
{
    ...
    *info = nvidia_platform_info->linfo[local_id];

    /* Get LED mode */
    if (mc_get_kernel_ver() >= KERNEL_VERSION(4,9,30)) {
        char* cmd = aim_fstrdup("%s%s_state", prefix_path, nvidia_platform_info->led_fnames[local_id]);
        if(system(cmd) != 0) {
            aim_free(cmd);
    ...
    if (onlp_file_read(data, sizeof(data), &len, "%s%s",
                       prefix_path, nvidia_platform_info->led_fnames[local_id]) != 0) {
        return ONLP_STATUS_E_INTERNAL;
    }
        ...
        if (mc_get_kernel_ver() < KERNEL_VERSION(4,9,30))
            ...
            if (led_colors_map[i].color)
                onlp_file_write((uint8_t*)LED_OFF, LED_MODE_LEN,
                                "%s%s_%s", prefix_path, nvidia_platform_info->led_fnames[id], led_colors_map[i].color);
        }
    }
...
onlp_ledi_mode_set(onlp_oid_t id, onlp_led_mode_t mode)
{
    ...
    if (mc_get_kernel_ver() < KERNEL_VERSION(4,9,30)) {
        ...
        if (onlp_file_write((uint8_t*)driver_led_mode, nbytes,
                            "%s%s", prefix_path, nvidia_platform_info->led_fnames[local_id]) != 0) {
            return ONLP_STATUS_E_INTERNAL;
        }
    } else {
