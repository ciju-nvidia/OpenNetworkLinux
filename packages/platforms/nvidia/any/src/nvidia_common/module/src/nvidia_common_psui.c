...
_psu_info_get_type2(onlp_psu_info_t* info)
{
    ...
    info->hdr.coids[0] = ONLP_FAN_ID_CREATE(index + nvidia_platform_info->fan_num);
    info->hdr.coids[1] = ONLP_THERMAL_ID_CREATE(index + nvidia_platform_info->thermal_num);
...
int _psu_info_get(onlp_psu_info_t* info)
{
    int res;
    nvidia_platform_info_t* nvidia_platform_info = get_platform_info();
    switch(nvidia_platform_info->psu_type) {
      case PSU_TYPE_1:
...
onlp_psui_info_get(onlp_oid_t id, onlp_psu_info_t* info)
{
    ...
    *info = pinfo[index]; /* Set the onlp_oid_hdr_t */
    ...
    nvidia_platform_info_t* nvidia_platform_info = get_platform_info();
       if(nvidia_platform_info->psu_fixed == false) {
