...
#include "x86_64_bull_xh3000_hse_switch_int.h"
#include "x86_64_bull_xh3000_hse_switch_log.h"
...
int mc_get_platform_info(mlnx_platform_info_t* mlnx_platform)
{
	if (!__ONL_PLATFORM_NAME) {
		aim_strlcpy(mlnx_platform->onl_platform_name, "x86-64-bull-xh3000-hse-switch-all", PLATFORM_NAME_MAX_LEN);
	}
	else {
...
int onlp_sysi_platform_set(const char* platform)
{
	...
	if (!strcmp(platform, "x86-64-bull-xh3000-hse-switch-r0")) {
		__ONL_PLATFORM_NAME = "x86-64-bull_xh3000_hse_switch-r0";
		mlnx_platform = get_platform_info();
		mc_get_platform_info(mlnx_platform);
		return ONLP_STATUS_OK;
	} else if (!strcmp(platform, "x86-64-bull-xh3000-hse-switch-all")) {
		__ONL_PLATFORM_NAME = "x86-64-bull-xh3000-hse-switch-all";
		return ONLP_STATUS_OK;
	}
