...
int mc_get_platform_info(mlnx_platform_info_t* mlnx_platform)
{
	if (!__ONL_PLATFORM_NAME) {
		aim_strlcpy(mlnx_platform->onl_platform_name, "x86-64-mlnx-msn4800-all", PLATFORM_NAME_MAX_LEN);
	}
	else {
		aim_strlcpy(mlnx_platform->onl_platform_name, __ONL_PLATFORM_NAME, PLATFORM_NAME_MAX_LEN);
	}

	if (!strcmp(mlnx_platform->onl_platform_name, "x86-64-mlnx_msn4800c-r0")) {
...
onlp_sysi_platform_set(const char* platform)
{
    ...
    if(!strcmp(platform, "x86-64-mlnx-msn4800-r0")) {
        __ONL_PLATFORM_NAME = "x86-64-mlnx_msn4800-r0";
        mlnx_platform = get_platform_info();
        mc_get_platform_info(mlnx_platform);
        return ONLP_STATUS_OK;
    } else if(!strcmp(platform, "x86-64-mlnx-msn4800c-r0")) {
        __ONL_PLATFORM_NAME = "x86-64-mlnx_msn4800c-r0";
        mlnx_platform = get_platform_info();
        mc_get_platform_info(mlnx_platform);
        return ONLP_STATUS_OK;
    } else if(!strcmp(platform, "x86-64-mlnx-msn4800-all")) {
        __ONL_PLATFORM_NAME = "x86-64-mlnx-msn4800-all";
        return ONLP_STATUS_OK;
    }
